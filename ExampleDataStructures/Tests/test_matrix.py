...
from ExampleDataStructures.Matrix import Matrix
...
class TestMatrix(TestCase):
    ...
    def test_pow_2(self):
        test_subject = Matrix.get_from_edges(3, {0: [1, 2], 1: [1, 2], 2: [1, 2]})
        self.assertListEqual(test_subject.pow(3).matrix, [[0, 4, 4], [0, 4, 4], [0, 4, 4]])
    ...
    def test_pow_3_mod_54975581(self):
        test_subject = Matrix.get_from_edges(6,
                                             {0: [1, 3], 1: [0, 2, 4], 2: [1, 5], 3: [0, 4], 4: [1, 3, 5], 5: [4, 2]})
        self.assertEqual(12, sum(test_subject.pow(3).matrix[0]))
